...
if [ "$1" = 'platform' ]; then
    ...
    then
        ...
        sed -Ei "s*DATABASE_URL*$DATABASE_URL*" $MM_CONFIG && echo "INJECTED $DATABASE_URL"
        sed -Ei "s/DB_HOST/$DB_HOST/" $MM_CONFIG
        sed -Ei "s/DB_PORT/$DB_PORT_3306_TCP_PORT/" $MM_CONFIG
    ...
    echo "Wait until database $DB_HOST:$DB_PORT_3306_TCP_PORT is ready..."
    until nc -z $DB_HOST $DB_PORT_3306_TCP_PORT
    do
        sleep 1
    done
